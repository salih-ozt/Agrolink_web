...
// 🔒 SecureDatabase v2.0 - Retry Mekanizmalı (SQLITE_BUSY çözümü)

/**
 * SecureDatabase Sınıfı - Retry Mekanizmalı
 * 
 * ✅ SQLITE_BUSY (Database Locked) hatalarını otomatik retry ile çözer
 * ✅ Prepared statement kullanarak SQL injection koruması sağlar
 * ✅ Yoğun trafik altında bile stabil çalışır
 */
class SecureDatabase {
    constructor(db, options = {}) {
        this.db = db;
        this.maxRetries = options.maxRetries || 10;          // Maksimum deneme sayısı
        this.retryDelay = options.retryDelay || 100;         // İlk bekleme süresi (ms)
        this.maxRetryDelay = options.maxRetryDelay || 5000;  // Maksimum bekleme süresi (ms)
    }
    
    // 🔄 Retry mekanizması ile sorgu çalıştırma
    async _executeWithRetry(operation, sql, params = []) {
        let lastError;
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                // Sorguyu çalıştır
                const result = await this.db[operation](sql, ...params);
                
                // Başarılı - loglama (sadece retry durumunda)
                if (attempt > 1) {
                    console.log(`✅ [SecureDB] Sorgu başarılı (${attempt}. deneme): ${sql.substring(0, 50)}...`);
                }
                ...
                // SQLITE_BUSY veya SQLITE_LOCKED hatası mı kontrol et
                ...
                if (isLockError && attempt < this.maxRetries) {
                    ...
                    const jitter = Math.random() * 50; // Rastgele jitter ekle
                    const waitTime = Math.min(delay + jitter, this.maxRetryDelay);
                    
                    console.warn(`⚠️ [SecureDB] Database kilitli, ${attempt}/${this.maxRetries} deneme, ${waitTime.toFixed(0)}ms bekleniyor...`);
                ...
                // Retry yapılmayacak hata veya max deneme aşıldı
                throw error;
            }
        }
        
        // Tüm denemeler tükendi
        console.error(`❌ [SecureDB] Tüm retry denemeleri tükendi (${this.maxRetries}x): ${sql.substring(0, 50)}...`);
        throw lastError;
    }
    ...
    // ✏️ INSERT/UPDATE/DELETE işlemleri
    ...
    async exec(sql) {
        return this._executeWithRetry('exec', sql, []);
    }
...
// 🔧 DÜZELTİLDİ: Rate limit süreleri makul hale getirildi
const API_SECURITY_CONFIG = {
    ...
    // Rate limiting - 🔧 MAKUL LİMİTLER v5.2
    maxConcurrentUsers: 500,           // Aynı anda maksimum 500 kullanıcı
    maxPostsPerMinute: 10,             // Dakikada maksimum 10 post
    postBanDurationMinutes: 5,         // 🔧 DÜZELTİLDİ: 1 saat -> 5 dakika (kullanıcı dostu)
    maxDuplicateUploads: 5,            // 🔧 DÜZELTİLDİ: 3 -> 5 (daha toleranslı)
    duplicateUploadBanMinutes: 10,     // 🔧 DÜZELTİLDİ: 30 -> 10 dakika
...
function checkPostRateLimit(userId) {
    ...
    if (count > API_SECURITY_CONFIG.maxPostsPerMinute) {
        // 🔧 DÜZELTİLDİ: 5 dakika engel (1 saat çok agresif!)
        const banDuration = API_SECURITY_CONFIG.postBanDurationMinutes * 60 * 1000;
        postBannedUsers.set(userId, now + banDuration);
        
        console.log(`🚫 KULLANICI GEÇİCİ ENGEL: ${userId} - ${API_SECURITY_CONFIG.postBanDurationMinutes} dakika post atamaz (${count} post/dakika)`);
        
        return { 
            allowed: false, 
            banned: true,
            count,
            message: `Dakikada maksimum ${API_SECURITY_CONFIG.maxPostsPerMinute} gönderi paylaşabilirsiniz. ${API_SECURITY_CONFIG.postBanDurationMinutes} dakika sonra tekrar deneyebilirsiniz.`
        };
    }
...
// ==================== CLUSTER BAŞLATMA ====================
// 🔧 DÜZELTİLDİ: SQLite çoklu process yazma desteği sunmaz (SQLITE_BUSY hatası)
// Cluster modu DEVRE DIŞI bırakıldı - tek process ile çalışır
// Eğer çoklu işlemci gerekiyorsa PostgreSQL veya MySQL'e geçiş yapılmalıdır

const DISABLE_CLUSTER_FOR_SQLITE = true; // SQLite ile cluster kullanılamaz!

if (!DISABLE_CLUSTER_FOR_SQLITE && cluster.isMaster && process.env.NODE_ENV === 'production') {
    console.log(`🚀 Master process başlatılıyor: ${process.pid}`);
    console.log(`🔢 ${numCPUs} CPU çekirdeği tespit edildi`);
    console.log(`⚠️ UYARI: SQLite ile cluster modu SQLITE_BUSY hatalarına neden olabilir!`);
    
    // Worker'ları fork et - MAX 2 WORKER (SQLite için güvenli)
...
} else if (DISABLE_CLUSTER_FOR_SQLITE) {
    console.log(`🔧 Cluster modu DEVRE DIŞI (SQLite uyumluluğu için)`);
    console.log(`📝 Tek process modunda çalışıyor: PID ${process.pid}`);
}

// 🔧 GLOBAL HATA YAKALAYICI - Beklenmeyen hatalar için
process.on('unhandledRejection', (reason, promise) => {
    console.error('❌ Yakalanmamış Promise Reddi:', reason);
    // Sunucuyu çökertmeden devam et
});

process.on('uncaughtException', (error) => {
    console.error('❌ Yakalanmamış İstisna:', error);
    // Kritik hatalarda 5 saniye sonra çık
    if (error.code === 'EADDRINUSE' || error.code === 'EACCES') {
        console.error('🔴 Port kullanımda veya erişim hatası, sunucu kapatılıyor...');
...
app.post('/api/posts', authenticateToken, checkRestriction, upload.array('media', UPLOAD_CONFIG.maxFilesPerUpload), handleMulterError, async (req, res) => {
    ...
    try {
        ...
        // 🔧 DÜZELTİLDİ: TÜM DOSYALARI İŞLE (sadece ilk dosya değil!)
        let mediaArray = [];
        ...
        if (req.files && req.files.length > 0) {
            console.log(`📁 ${req.files.length} dosya işleniyor (TÜM DOSYALAR)...`);
            
            // 🔧 DÜZELTİLDİ: TÜM dosyaları işle, sadece ilkini değil!
            for (let fileIndex = 0; fileIndex < req.files.length; fileIndex++) {
                const file = req.files[fileIndex];
                
                // 🔧 DÜZELTİLDİ: application/octet-stream da video olabilir
                const videoMimeTypes = ['video/', 'application/octet-stream'];
                const videoExtensions = ['.mp4', '.mov', '.avi', '.webm', '.mkv', '.m4v', '.mpeg', '.3gpp'];
                const fileExt = path.extname(file.originalname).toLowerCase();
                
                const isVideo = file.mimetype.startsWith('video/') || 
                               (file.mimetype === 'application/octet-stream' && videoExtensions.includes(fileExt));
                ...
                if (isVideo) {
                    // VİDEO İŞLEME
                    const filename = `video_${timestamp}_${randomSuffix}_${fileIndex}.mp4`;
                    const outputPath = path.join(videosDir, filename);
                    
                    console.log(`🎬 Video [${fileIndex + 1}/${req.files.length}]: ${file.originalname} -> ${filename}`);
                    ...
                    try {
                        ...
                        mediaArray.push(`/uploads/videos/${filename}`);
                        finalMediaType = 'video';
                        
                        // 🔧 DÜZELTİLDİ: Thumbnail oluştur ve hatayı düzgün logla
                        const thumbPath = path.join(videosDir, `thumb_${filename.replace('.mp4', '.jpg')}`);
                        createVideoThumbnail(outputPath, thumbPath)
                            .then(success => {
                                if (!success) {
                                    console.warn(`⚠️ Thumbnail oluşturulamadı (video yine de kullanılabilir): ${filename}`);
                                }
                            })
                            .catch(err => {
                                console.error(`❌ Thumbnail hatası (video yine de kullanılabilir): ${err.message}`);
                            });
                        
                        console.log(`✅ Video kaydedildi [${fileIndex + 1}]: ${filename}`);
                    } catch (err) {
                        console.error(`❌ Video hatası [${fileIndex + 1}]: ${err.message}`);
                        // Hata durumunda dosyayı temizle ve devam et
                        await fs.unlink(file.path).catch(() => {});
                    }
                } else {
                    // RESİM İŞLEME
                    const filename = `img_${timestamp}_${randomSuffix}_${fileIndex}.webp`;
                    const outputPath = path.join(postsDir, filename);
                    
                    console.log(`📷 Resim [${fileIndex + 1}/${req.files.length}]: ${file.originalname} -> ${filename}`);
                    ...
                    try {
                        ...
                        mediaArray.push(`/uploads/posts/${filename}`);
                        if (finalMediaType === 'text') finalMediaType = 'image';
                        
                        console.log(`✅ Resim kaydedildi [${fileIndex + 1}]: ${filename}`);
                    } catch (err) {
                        console.error(`❌ Resim hatası [${fileIndex + 1}]: ${err.message}`);
                        // Sharp başarısız olursa direkt kopyala
                        try {
                            const fallbackFilename = `img_${timestamp}_${randomSuffix}_${fileIndex}.jpg`;
                            const fallbackPath = path.join(postsDir, fallbackFilename);
                            await fs.copyFile(file.path, fallbackPath);
                            await fs.unlink(file.path).catch(() => {});
                            mediaArray.push(`/uploads/posts/${fallbackFilename}`);
                            if (finalMediaType === 'text') finalMediaType = 'image';
                            console.log(`✅ Resim direkt kopyalandı [${fileIndex + 1}]`);
                        } catch (copyErr) {
                            console.error(`❌ Kopyalama da başarısız [${fileIndex + 1}]: ${copyErr.message}`);
                            await fs.unlink(file.path).catch(() => {});
                        }
                    }
                }
            ...
            // 🔧 DÜZELTİLDİ: Artık dosyaları silmiyoruz, hepsi işlendi!
            // ESKİ HATALI KOD:
        ...
        // Media değişkenini ayarla (tek dosya ise string, birden fazla ise JSON array)
        let media = null;
...
// 🔧 DÜZELTİLDİ: Cluster devre dışı olduğunda da sunucu başlatılmalı
if (DISABLE_CLUSTER_FOR_SQLITE || cluster.isWorker || process.env.NODE_ENV !== 'production') {
    startServer();
} else {
    console.log('⚠️ Sunucu başlatılmadı - bu sadece master process');
}
