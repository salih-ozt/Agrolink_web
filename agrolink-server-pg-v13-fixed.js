...
app.post('/api/auth/forgot-password', async (req, res) => {
    try {
        ...
        if (user) {
            ...
            // ✅ DÜZELTME: Eski tokenları sil (birden fazla geçerli token karışıklık yaratıyor)
            await pool.query(`DELETE FROM password_resets WHERE "userId" = $1`, [user.id]).catch(() => {});

            // ✅ DÜZELTME: expiresAt'i JS Date değil PostgreSQL interval ile hesapla
            // → Sunucu saati / timezone farkından etkilenmez
            await dbRun(
